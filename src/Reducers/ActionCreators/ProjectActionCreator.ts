...
export const ProjectsActionCreators = () => {
    return async (dispatch: Dispatch<ProjectAction>) => {
        await projectApi.projects()
            .then((response) => {
                // @ts-ignore
                dispatch({type: ProjectUserTypes.GET_PROJECTS, status: response.status, error: null, projects: response.data})
            })
            .catch((e) => {
...
export const ProjectActionCreator = (id: string) => {
    return async (dispatch: Dispatch<ProjectAction>) => {
        await projectApi.project(id)
            .then((response) => {
                // @ts-ignore
                dispatch({type: ProjectUserTypes.GET_PROJECT, status: response.status, error: null, project: response.data})
            })
            .catch((e) => {
